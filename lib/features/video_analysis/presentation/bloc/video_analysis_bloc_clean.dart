...
class VideoAnalysisBloc extends Bloc<VideoAnalysisEvent, VideoAnalysisState> {
  ...
  Future<void> _onLoadVideoHistory(
    ...
    result.fold(
      (failure) =>
          emit(const VideoAnalysisError('Failed to load video history')),
  ...
  Future<void> _onSelectVideo(
    ...
    historyResult.fold(
      (failure) => emit(
        VideoSelected(selectedVideo: event.video, videoHistory: [event.video]),
      ),
      (history) => emit(
        VideoSelected(selectedVideo: event.video, videoHistory: history),
      ),
    );
  }
    ...
    await result.fold(
      ...
      (_) async {
        ...
        if (!emit.isDone) {
          historyResult.fold(
            (failure) => emit(
              const VideoAnalysisError('Failed to load updated history'),
            ),
